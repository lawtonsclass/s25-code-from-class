...
# where the ball should be/where it should move
...
def drawFrame():
    ...
    if turtle_y <= -200 + turtle_width/2:
        # we either hit the bottom wall, or we're trying
        # to go below it
        # move the turtle back inside
        turtle_y = -200 + turtle_width/2
        # change the turtle_velocity so we move upwards now
        # turtle_velocity = turtle_velocity * -1
        # create a perfectly elastic collision by hard-coding the new velocity
        turtle_velocity = 19.0

    if turtle_y >= 200 - turtle_width/2:
        # we either hit the top wall, or we're trying
        # to go above it
        # move the turtle back inside
        turtle_y = 200 - turtle_width/2
        # change the turtle_velocity so we move upwards now
